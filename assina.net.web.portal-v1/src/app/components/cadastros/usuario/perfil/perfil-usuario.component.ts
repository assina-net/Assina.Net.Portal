...
export class PerfilUsuarioComponent extends PadraoNovoComponent {
  ...
  constructor(route: ActivatedRoute,
    ...
    this.titulo = "Perfil de Usuário";
    this.navegacao = " > Cadastro > Perfil > Usuario";
    this.rota = undefined;
  ...
  get cliente() {
    if (this.entidade == null) {
      return { id: "", pessoa: new Pessoa('', '', '', '', TipoPessoaEnum.FISICA, '', undefined) };
    }
  ...
  salvar(validar?: boolean) {
    ...
    this.httpService.saveUpdate(this.entidade).subscribe((responseApi: ResponseApi) => {
       if (this.afterSave(responseApi)) {
          this.entidade.usuario = responseApi;
  ...
  findById(id: string) {
    ...
    let usuarioCliente = { usuario: { id: this.shared.usuario.id }, cliente: { id: this.shared.usuario.pessoa.cliente.id } };
